 ...
 for(const preset of Object.values(LADDER_PRESETS)){const definition={...preset,...(p.id.startsWith('pig')?{exitWidth:WIDE_LADDER_EXIT}:{})};
  ...
  try{
   for(const phase of m.phases)for(let i=0;i<=16;i++){
    const progress=(phase.start+(phase.end-phase.start)*i/16)/m.duration,r=m.apply(progress),joints=w.bones.map(b=>b.getWorldPosition(new T.Vector3())),previous=rows.at(-1);rows.push({progress,joints});
    ...
    if(previous&&progress>previous.progress){const dt=(progress-previous.progress)*m.duration;assert.ok(joints.every((q,i)=>q.distanceTo(previous.joints[i])<dt*4+.005),'joint discontinuity '+JSON.stringify({phase:r.phase,progress,dt,jumps:joints.map((q,i)=>[w.bones[i].name,q.distanceTo(previous.joints[i])]).filter(q=>q[1]>=dt*4+.005)}));}
