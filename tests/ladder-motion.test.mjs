...
for(const p of profiles.filter(p=>!p.unarmed&&(!process.env.REVIEW_ANIMAL||process.env.REVIEW_ANIMAL===p.id)))test(p.id+': ladder contacts, surfaces, timing, reverse playback and cancellation',()=>{
 for(const preset of Object.values(LADDER_PRESETS)){const definition={...preset,...(p.id.startsWith('pig')?{exitWidth:WIDE_LADDER_EXIT}:{})};
  ...
  try{
   assert.equal(m.duration,10);assert.equal(m.phases[0].start,0);assert.equal(m.phases.at(-1).end,10);assert.ok(Math.abs(m.phases.at(-4).start-8.6)<1e-10);assert.ok(Math.abs(m.phases.at(-4).end-m.phases.at(-4).start-.45)<1e-10);assert.ok(Math.abs(m.phases.at(-3).end-m.phases.at(-3).start-.45)<1e-10);
   for(const phase of m.phases)for(let i=0;i<=16;i++){
    const progress=(phase.start+(phase.end-phase.start)*i/16)/m.duration,r=m.apply(progress),joints=w.bones.map(b=>b.getWorldPosition(new T.Vector3())),previous=rows.at(-1);assert.ok(Math.abs(r.time-progress*10)<1e-8);assert.equal(r.duration,10);rows.push({progress,joints,poseTime:r.poseTime});
