...
function event(rotated=false,slot=0,direction='up',z=0,kind='iron-searchlight-ladder-tower'){const tower={kind,x:10,y:10,z,rotated},bottom=towerEntry(tower),top=towerSlots(tower)[slot];top.towerPost=towerPost(tower,top);return {id:1,unitId:0,access:'ladder',tower,direction,from:direction==='up'?bottom:top,to:direction==='up'?top:bottom};}
...
for(const kind of ['iron-searchlight-ladder-tower','wooden-spotlight-tower'])for(const id of ['skunk','dog'])test(id+' '+kind+': upper tail pose matches ordinary standing, restores normals and never changes paint coordinates',()=>{
 ...
 try{for(const direction of ['up','down']){
  const e=event(true,3,direction,0,kind),m=createLadderJourney(w,p,e,ladderFrame(e.tower),273),upper=direction==='up'?1:0;
