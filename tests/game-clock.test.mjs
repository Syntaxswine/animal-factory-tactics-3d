...
test('one shared minute clock rolls through dusk, night, midnight and day independently of frame size',()=>{
 const a=createClock(1079),b=createClock(1079);advanceClock(a,elapsedGameMinutes(1000));for(let i=0;i<60;i++)advanceClock(b,elapsedGameMinutes(1000/60));assert.equal(formatClock(a),formatClock(b));assert.equal(timeOfDay(a).phase,'dusk');
...
test('a full 3D round is exactly one minute; thinking, animations and repeated observations charge nothing',()=>{
 ...
 s.units.filter(u=>u.team==='guard').forEach(g=>g.hp=0);refresh(s);assert.equal(settleEncounterRounds(s),1);assert.equal(settleEncounterRounds(s),0);tickEncounterClock(s,1000);assert.equal(s.clock.minutes,1202);
