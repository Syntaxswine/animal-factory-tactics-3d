...
test('fixtures have downward passive emitter anchors and no active lighting',()=>{
 ...
 for(const f of FURNITURE_FORMS){const {root}=library.build(f.id);const emitters=[];
  ...
  for(const a of emitters){assert.equal(a.userData.enabled,false);const direction=new THREE.Vector3(0,0,-1).applyQuaternion(a.getWorldQuaternion(new THREE.Quaternion()));assert(direction.y<(['wooden-spotlight-tower','iron-searchlight-stair-tower','iron-searchlight-ladder-tower'].includes(f.id)?-.15:-.999));if(['wooden-spotlight-tower','iron-searchlight-stair-tower','iron-searchlight-ladder-tower'].includes(f.id)){assert(direction.z>.9);assert.equal(a.userData.distribution,'spot');}}
