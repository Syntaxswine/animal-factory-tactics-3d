...
The [3D map editor](docs/tactics/THREED-EDITOR-EDITING.md) at `tactics/editor-3d.html` now supports brush previews, terrain/room/object/start tools, selection, rotation, deletion, undo/redo, named browser saves, JSON exchange and playtesting the edited snapshot. Five camera views and three floors remain available. Blocks can be inspected and exported; use the existing editor for block assembly and generation.
