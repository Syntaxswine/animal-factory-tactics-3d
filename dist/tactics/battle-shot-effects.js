...
export function shotPoint(p){const z=Math.max(0,Math.floor((p.h||0)/3));return new T.Vector3(p.x,z*DIMENSIONS.floorSpacing+((p.h||0)-z*3)*DIMENSIONS.standing/1.8,p.y);}
const visible=(state,p)=>{const z=Math.max(0,Math.min(2,Math.floor(p.y/DIMENSIONS.floorSpacing))),x=Math.round(p.x),y=Math.round(p.z);return state.visible.has(z?`${x},${y},${z}`:`${x},${y}`);};
...
export class BattleShotEffects {
 ...
 update(active,state,muzzle){
  ...
  if(muzzle&&phase.trace){
   const path=visibleShotPath(state,[active.traceOrigin||muzzle.origin,...(shot.path||[shot]).map(shotPoint)]);
