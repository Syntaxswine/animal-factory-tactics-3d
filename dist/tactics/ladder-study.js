...
let worker=null,motion=null,paint=null,cap=null,ladder=null,loading=false,playing=false,last=0;
...
window.ladderStudy={ready:false,seek(p){$('progress').value=p;render();},render};for(const id of ['animal','outfit','ladder'])$(id).onchange=load;for(const id of ['view','scale','direction','progress'])$(id).oninput=render;$('play').onclick=()=>{playing=!playing;$('play').textContent=playing?'Pause':'Play';if(+$('progress').value===1)$('progress').value=0;last=performance.now();};
function frame(now){if(playing&&motion){$('progress').value=Math.min(1,+$('progress').value+(now-last)/1000/motion.duration);render();if(+$('progress').value>=1){playing=false;$('play').textContent='Play';}}last=now;requestAnimationFrame(frame);}requestAnimationFrame(frame);load();
