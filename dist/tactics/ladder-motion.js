...
export function createLadderMotion(worker,profile,definition=LADDER_PRESETS.floor){
 ...
 const grips=limbs.filter(l=>l.finger).map(l=>({limb:l,asset:createLadderGrip(worker,l.side,rest.get(l.c).y,{wooden:!!d.hatch})}));
 if(d.hatch){
 ...
 while(fi[-1]<lastFoot||fi[1]<lastFoot)for(const side of [-1,1]){
  ...
  hi[side]+=advance;const hand=rung(hi[side],side,true);hand.point.y=Math.min(hand.point.y,d.hatch?d.height+.77:d.railTop-.005);add('Reach '+(side===-1?'left':'right'),'hand'+side,hand,profile.id.startsWith('pig')?1.5:1.1);
 }
 ...
 const hatchBoxes=[];if(d.hatch){const f=d.hatch.front,b=d.hatch.back,box=(x,y,z,w,h,l)=>hatchBoxes.push([x,y,z,w/2,h/2,l/2]);for(const side of [-1,1]){for(const y of [.47,.91])box((f+b)/2,d.height+y,side*.58,1.04,.085,.085);for(const x of [f-.02,b+.02])box(x,d.height+.46,side*.58,.085,.92,.085);box((f+b)/2,d.height-.15,side*.58,1.12,.18,.16);box((f+b)/2,d.height-.055,side*1.5,1,.11,2);}for(const y of [.47,.91])box(b+.02,d.height+y,0,.085,.085,1.16);for(const [x,len]of [[f+1.875,3.75],[b-.125,.25]])box(x,d.height-.055,0,len,.11,5);}
 ...
 function applyPose(progress,{direction='up',heading=0,origin=[0,0,0]}={}){
  ...
  const position=desired.clone();for(let n=0;n<400;n++){let error=0;for(const c of constraints){const v=position.clone().sub(c.center),len=v.length();if(len>c.radius){position.copy(c.center).addScaledVector(v,c.radius/len);error=Math.max(error,len-c.radius);}else if(len<c.minRadius){position.copy(c.center).addScaledVector(len>1e-8?v.normalize():V(-1,0,0),c.minRadius);error=Math.max(error,c.minRadius-len);}}if(heavy){const clearance=mounted*(1-T.MathUtils.smoothstep(position.y,d.height-.65,d.height-.45)),limit=T.MathUtils.lerp(d.plane+1.2,-.14,clearance);if(position.x>limit){error=Math.max(error,position.x-limit);position.x=limit;}}if(d.hatch&&position.y>d.height-1.2){const minX=.50*ease((t-hatchAdvanceStart)/1.4);error=Math.max(error,Math.abs(position.z),minX-position.x);position.z=0;position.x=Math.max(position.x,minX);}if(error<1e-8)break;}
