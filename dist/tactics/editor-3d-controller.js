...
import {cutCliffRamp,outerRampBanks} from './cliff-ramp-placement.js';
...
export class EditingDocument extends InspectionDocument {
 ...
 preview(command){
  ...
  try{
   if(tool==='ramp-cut'){
    const result=cutCliffRamp(candidate.map,start,options);
    if(this.block&&result.cells.some(p=>p.x<0||p.y<0||p.x>=24||p.y>=24))throw Error('The complete ramp must fit inside the block.');
    if(this.block)validateBlock(extractBlock(result.map));
    const errors=validateMap(result.map,{connectivity:false});
    return {ok:!errors.length,error:errors[0]||'',cells:result.cells,edges:[],map:result.map};
   }
