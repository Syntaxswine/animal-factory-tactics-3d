import {canSpendStamina,spendStamina} from './stamina.js';
...
export function towerClimbPreview(s,u){
 ...
 let reason='';if(!canControl(s,u)||s.queue.length)reason='Cannot climb now.';else if(!canSpendStamina(u,12))reason='Climbing needs 12 stamina.';else if(u.stance!=='standing')reason='Stand up to climb.';else if(combatCosts(s)&&u.ap<TOWER_CLIMB_AP)reason='Climbing needs 6 AP.';else if(!destination||!vacant(s,destination))reason=occupied?'Tower exit is occupied.':'All tower posts are occupied.';else if(occupied&&!passable(s,destination))reason='Tower exit is blocked.';
...
export function climbTower(s,u){const r=towerClimbPreview(s,u);if(!r.ok)return false;
 ...
 spendStamina(u,12);Object.assign(u,r.destination);if(r.descending)delete u.towerPost;else u.towerPost=towerPost(r.tower,r.destination);
