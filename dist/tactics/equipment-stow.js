...
export function createEquipmentStow(worker,profile,{riflePlacement}={}){
 ...
 function apply(){if(disposed)throw Error('Equipment stow is disposed');material.opacity=1;material.transparent=false;if(worker.weapon!==gun)throw Error('Equipment changed during stow');if(state!=='stowed'){saved=save();state='stowed';}root.updateMatrixWorld(true);const parentQ=root.getWorldQuaternion(new T.Quaternion()).invert(),spineQ=spine.getWorldQuaternion(new T.Quaternion()).premultiply(parentQ),position=V(),axis=V(0,.9,profile.id==='skunk'?-.12:.42).normalize();
  ...
  let anchor=center;
  if(id==='rifle'){position.set(back,-.23,z);anchor=gun.anchors.stock.position;if(riflePlacement?.axis)axis.fromArray(riflePlacement.axis);}
  ...
  const origin=root.worldToLocal(spine.localToWorld(position.clone()));gun.root.visible=true;gun.root.quaternion.setFromUnitVectors(V(1,0,0),axis.normalize()).premultiply(spineQ);if(id==='rifle'&&riflePlacement?.preserveAxisRoll)gun.root.quaternion.setFromUnitVectors(V(1,0,0),axis.normalize().applyQuaternion(spineQ));gun.root.position.copy(origin).sub(anchor.clone().applyQuaternion(gun.root.quaternion));
