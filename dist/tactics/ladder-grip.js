...
export function createLadderGrip(worker,side,handHeight,{wooden=false}={}){
 ...
 for(const z of [-.035,-.012,.012,.035]){
  ...
  const curve=wooden?new T.CatmullRomCurve3(Array.from({length:13},(_,i)=>{const a=(170-i*20)*Math.PI/180;return V(.083*Math.cos(a),.083*Math.sin(a),z);})):path;
  const mesh=add(new T.TubeGeometry(curve,wooden?24:18,.010,5,false));if(wooden)fingers.push({mesh,source:mesh.geometry.attributes.position.clone(),thumb:false});
 }
 const thumb=new T.CatmullRomCurve3([V(-.080,.01,side*.041),V(-.062,-.015,side*.057),V(-.035,-.041,side*.052),V(-.01,-.046,side*.038)]);
 const thumbCurve=wooden?new T.CatmullRomCurve3([V(-.085,.006,side*.041),V(-.088,-.025,side*.057),V(-.073,-.060,side*.052),V(-.034,-.083,side*.038)]):thumb;
 const thumbMesh=add(new T.TubeGeometry(thumbCurve,10,.014,6,false));if(wooden)fingers.push({mesh:thumbMesh,source:thumbMesh.geometry.attributes.position.clone(),thumb:true});
