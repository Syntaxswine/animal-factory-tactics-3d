...
export class BattleTraversal {
 constructor(prepare=null){this.prepare=prepare;this.lastId=0;this.active=null;this.queue=[];}
 ...
 pose(model,unit,now){const a=this.active;if(!a||a.event.unitId!==unit.id)return false;
  ...
  if(!a.motion){worker.root.position.set(0,0,0);worker.root.rotation.set(0,0,0);root.updateMatrixWorld(true);model.posture?.resetTail();a.weapon=unit.weapon;a.motion=createLadderJourney(worker,profile,a.event,ladderFrame(a.event.tower),unit.heading||0,{preparedOnly:!!this.prepare});a.model=model;a.start=now;}
