...
async function start(){
 ...
 function rebuild(){cliffs.forEach(c=>{scene.remove(c.root);c.dispose();});clearGuides();cliffs=[];const set=$('set').value,sets=set==='both'?['ledge','crag']:[set],layout=$('layout').value;
  $('shape').disabled=layout!=='single';
  $('ledge-legend').hidden=set==='crag';$('crag-legend').hidden=set==='ledge';
  function add(c,x,z=0){c.original=c.mesh.material;c.root.position.set(x,0,z);scene.add(c.root);cliffs.push(c);}
  sets.forEach((kind,i)=>{
 ...
 for(const id of ['layout','shape','set'])$(id).onchange=rebuild;for(const id of ['scale','grey','guides','wire'])$(id).onchange=render;$('variant').onclick=()=>{seed++;rebuild();};$('reset').onclick=()=>{az=GAME_CAMERA.azimuth;el=GAME_CAMERA.elevation;render();};
