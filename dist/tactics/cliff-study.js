...
async function start(){
 ...
 function rebuild(){cliffs.forEach(c=>{scene.remove(c.root);c.edgeRocks?.dispose();c.dispose();});clearGuides();cliffs=[];const set=$('set').value,sets=set==='both'?['ledge','crag']:[set],layout=$('layout').value;
  ...
  sets.forEach((kind,i)=>{
   ...
   if(layout==='atlas'){for(let mask=0;mask<16;mask++){add(createCliffTiles(kind,[paintTile({x:0,z:0,mask,variant:(seed-1)%3,...(kind==='mixed'?{set:mask%2?'crag':'ledge'}:{})})]),offset+(mask%4-1.5)*1.4-.5,(Math.floor(mask/4)-1.5)*1.4-.5);}return;}
   if(layout!=='single'){add(createCliffTiles(kind,cliffLandscapeTiles(layout,(seed-1)%3).map(t=>kind==='mixed'?{...t,set:t.x<0?'ledge':'crag'}:t).map(paintTile)),offset);for(let x=-5;x<=5;x++)line([[offset+x,2.01,-4],[offset+x,2.01,4]],0x8a9472);for(let z=-4;z<=4;z++)line([[offset-5,2.01,z],[offset+5,2.01,z]],0x8a9472);return;}
 ...
 for(const id of ['layout','shape','set','paint'])$(id).onchange=rebuild;for(const id of ['scale','grey','guides','wire','rocks','grass','sand'])$(id).onchange=render;$('variant').onclick=()=>{seed++;rebuild();};$('reset').onclick=()=>{az=GAME_CAMERA.azimuth;el=GAME_CAMERA.elevation;render();};
 ...
 const resize=new ResizeObserver(render);resize.observe(host);rebuild();window.cliffStudy={ready:true,scene,camera,renderer,horse,ground,sandPatch,get cliffs(){return cliffs;},view(a,e){az=a;el=e;render();},render};
