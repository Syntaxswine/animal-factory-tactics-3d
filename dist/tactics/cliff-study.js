...
import {createPaintedSand} from './painted-sand.js';
import {loadCliffWaterTexture,createCliffWater} from './cliff-water.js';
...
async function start(){
 if(new URLSearchParams(location.search).get('base')==='water')$('base').value='water';$('waves').checked=!matchMedia('(prefers-reduced-motion: reduce)').matches;
 ...
 const sandPatch=new T.Mesh(new T.PlaneGeometry(4,3),createPaintedSand({feather:true}));sandPatch.rotation.x=-Math.PI/2;sandPatch.position.set(-2.4,-.009,5.3);sandPatch.receiveShadow=true;scene.add(sandPatch);
 const waterTexture=await loadCliffWaterTexture(),water=createCliffWater(waterTexture);scene.add(water.mesh);
 ...
 function rebuild(){cliffs.forEach(c=>{scene.remove(c.root);c.edgeRocks?.dispose();c.dispose();});clearGuides();cliffs=[];const set=$('set').value,sets=set==='both'?['ledge','crag']:[set],layout=$('layout').value,wet=$('base').value==='water',options={water:wet};
  $('shape').disabled=layout!=='single';$('paint').disabled=layout==='single';$('set').querySelector('[value=mixed]').disabled=layout==='single';if(layout==='single'&&set==='mixed'){$('set').value='both';rebuild();return;}
  ...
  const paintTile=t=>({...t,...($('paint').value==='natural'?{}:{sand:$('paint').value==='sand'?1:0})});
  sets.forEach((kind,i)=>{
   const offset=sets.length===2?(i===0?-1:1)*(layout==='single'?3.1:layout==='atlas'?3.5:6):0;
   if(layout==='atlas'){for(let mask=0;mask<16;mask++){add(createCliffTiles(kind,[paintTile({x:0,z:0,mask,variant:(seed-1)%3,...(kind==='mixed'?{set:mask%2?'crag':'ledge'}:{})})],options),offset+(mask%4-1.5)*1.4-.5,(Math.floor(mask/4)-1.5)*1.4-.5);}return;}
   if(layout!=='single'){add(createCliffTiles(kind,cliffLandscapeTiles(layout,(seed-1)%3).map(t=>kind==='mixed'?{...t,set:t.x<0?'ledge':'crag'}:t).map(paintTile),options),offset);for(let x=-5;x<=5;x++)line([[offset+x,2.01,-4],[offset+x,2.01,4]],0x8a9472);for(let z=-4;z<=4;z++)line([[offset-5,2.01,z],[offset+5,2.01,z]],0x8a9472);return;}
   const c=createCliff(kind,$('shape').value,seed,options);add(c,offset);const x=c.root.position.x,{width:w,depth:d,climb}=c.layout;
   line([[x-w/2-.2,0,d/2+.2],[x-w/2-.2,CLIFF_HEIGHT,d/2+.2],[x+w/2+.2,CLIFF_HEIGHT,d/2+.2]],0xe8d79c);
   if(climb&&!wet){line([climb.approach,climb.lip,climb.landing].map(([a,b,c])=>[a+x,b+.025,c]),0xb3e684);const ring=new T.Mesh(new T.RingGeometry(.27,.30,32),new T.MeshBasicMaterial({color:0xb3e684,side:T.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.set(x+climb.landing[0],CLIFF_HEIGHT+.014,climb.landing[2]);guides.add(ring);}
  });const single=layout==='single';wall.position.set(single?(set==='both'?0:3.5):set==='both'?0:6,CLIFF_HEIGHT/2,single?.4:4.5);horse.root.position.set(single?(set==='both'?0:2.8):set==='both'?0:6,0,single?2.1:6);if(wet)water.setCliffs(cliffs.map(c=>c.mesh));render();
 }
 function render(){const w=host.clientWidth,h=host.clientHeight,single=$('layout').value==='single',span=single?13:$('set').value==='both'?27:18,ppu=$('scale').value==='auto'?Math.min(w/span,h/(single?7:13)): +$('scale').value;renderer.setSize(w,h,false);camera.left=-w/ppu/2;camera.right=w/ppu/2;camera.top=h/ppu/2;camera.bottom=-h/ppu/2;camera.near=.1;camera.far=60;camera.position.copy(focus).add(new T.Vector3(Math.sin(az)*Math.cos(el),Math.sin(el),Math.cos(az)*Math.cos(el)).multiplyScalar(25));camera.lookAt(focus);camera.updateProjectionMatrix();const wet=$('base').value==='water';water.mesh.visible=wet;ground.visible=!wet;horse.root.visible=wall.visible=!wet;guides.visible=$('guides').checked;grid.visible=$('guides').checked&&!wet;sandPatch.visible=$('sand').checked&&!wet;$('sand').disabled=wet;$('waves').disabled=!wet;const tint=$('grass').value;ground.material.userData.grassTint.value.set(tint);
  for(const c of cliffs){for(const m of c.original||[])m.userData.grassTint?.value.set(tint);c.mesh.material=$('grey').checked?grey:c.mesh.material===grey?c.original:c.mesh.material;c.original??=c.mesh.material;for(const m of [].concat(c.mesh.material))m.wireframe=$('wire').checked;if(c.edgeRocks)c.edgeRocks.root.visible=$('rocks').checked;}renderer.render(scene,camera);
 ...
 for(const id of ['layout','shape','set','paint','base'])$(id).onchange=rebuild;for(const id of ['scale','grey','guides','wire','rocks','grass','sand','waves'])$(id).onchange=render;$('variant').onclick=()=>{seed++;rebuild();};$('reset').onclick=()=>{az=GAME_CAMERA.azimuth;el=GAME_CAMERA.elevation;render();};
 ...
 let frame,lastTime=performance.now(),waterTime=0;function animate(now){const dt=Math.min(.1,(now-lastTime)/1000);lastTime=now;if(!document.hidden&&water.mesh.visible&&$('waves').checked){waterTime+=dt;water.update(waterTime);renderer.render(scene,camera);}frame=requestAnimationFrame(animate);}frame=requestAnimationFrame(animate);
 const resize=new ResizeObserver(render);resize.observe(host);rebuild();window.cliffStudy={ready:true,scene,camera,renderer,horse,ground,sandPatch,water,get cliffs(){return cliffs;},view(a,e){az=a;el=e;render();},render};
 addEventListener('pagehide',()=>{cancelAnimationFrame(frame);resize.disconnect();water.dispose();waterTexture.dispose();clearGuides();cliffs.forEach(c=>{c.edgeRocks?.dispose();c.dispose();});grey.dispose();sandPatch.geometry.dispose();sandPatch.material.dispose();ground.geometry.dispose();ground.material.dispose();wall.geometry.dispose();wall.material.dispose();grid.geometry.dispose();grid.material.dispose();paint.dispose();atlas.dispose();horse.skeleton.dispose();horse.dispose();renderer.dispose();},{once:true});
}
