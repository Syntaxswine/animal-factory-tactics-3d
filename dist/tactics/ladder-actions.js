...
export function climbPreview(state,unit,preferredLevel){
 ...
 const candidates=connections.map(p=>({p,to:{x:p.x,y:p.y,z:p.z===z?z+1:z-1}})).sort((a,b)=>(b.to.z===preferredLevel)-(a.to.z===preferredLevel)||b.to.z-a.to.z);
 if(!candidates.length)return {...tower,kind:'ladder',label:'Climb ladder',reason:'Stand on a ladder tile or at a tower entrance.'};
 const {p,to}=candidates[0],step=movementNeighbors(state,unit).find(q=>q.x===to.x&&q.y===to.y&&q.z===to.z);
 const reason=!canControl(state,unit)||state.queue.length?'Cannot climb now.':unit.stance!=='standing'?'Stand up to climb.':!step?'Ladder landing is blocked.':occupant(state,to.x,to.y,to.z)?'Landing is occupied.':!canMoveStamina(unit,step)?'Not enough stamina.':combatCosts(state)&&unit.ap<step.cost?'Not enough AP.':'';
 return {ok:!reason,reason,kind:'ladder',destination:to,cost:step?.cost??(p.kind==='ladder'?3:2),label:(to.z>z?'Climb ':'Descend ')+(p.kind==='ladder'?'ladder':'stairs')};
}
