...
export class BattleMotion {
 ...
 update(units,now,reduced=false){
  ...
  for(const u of units){
   ...
   if(!t){t={from:point,to:point,start:now,last:now,position:point,distance:0,blend:0,posture:postureOf(u),pose:weights(postureOf(u)),poseFrom:weights(postureOf(u)),poseStart:now};this.tracks.set(u.id,t);}
