...
export class HybridRenderer{
 ...
 draw(ctx,state,view,width,height,level,{editor=false}={}){
  ...
  this.prune();this.animateMaterials(performance.now()/1000);this.daylight?.update(this.state?.clock?.minutes??480,camera,this.presentationNow??0);this.renderer.render(this.scene,camera);ctx.drawImage(this.renderer.domElement,0,0,width,height);this.drawTimes??=[];this.drawTimes.push(performance.now()-drawStart);if(this.drawTimes.length>120)this.drawTimes.shift();return picking;
 }
