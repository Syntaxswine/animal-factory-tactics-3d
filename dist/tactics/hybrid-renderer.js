...
export class HybridRenderer{
 ...
 rebuild(world,seen,level,map){
  ...
  for(const box of environmentVisuals(world,map)){const z=box.source.z??(box.source.edge?Number(box.source.edge.split(':')[3]||0):0);if(z>level||!known(box,seen))continue;const material=materialKind(box),wall=!!this.wallXray&&box.kind==='wall',groupKey=`${wall?"xray:":""}${material}:${box.shape||'box'}:${Math.floor(box.center[0]/16)},${Math.floor(box.center[2]/16)}`;if(!groups.has(groupKey))groups.set(groupKey,{material,wall,shape:box.shape||'box',boxes:[]});groups.get(groupKey).boxes.push(box);}
