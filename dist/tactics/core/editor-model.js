...
export function applyBrush(editor,tool,x,y,edge,options={}) {
 ...
 if(tool==='roof'||tool==='cliff'){if(z>=2)return 'Roof climbs begin on level 1 or 2.';if(!/^(e|s):-?\d+:-?\d+(?::[12])?$/.test(edge))return 'Choose a tile edge.';const cells=edgeCells(edge),other=cells.find(p=>p.x!==x||p.y!==y);if(!other||!cells.some(p=>p.x===x&&p.y===y&&p.z===z))return 'Choose the edge beside the lower foothold.';const p={x,y,z,dx:other.x-x,dy:other.y-y,...(tool==='cliff'?{kind:'cliff'}:{})};if(!roofValid(m,p))return 'Paint an adjacent upper roof first. Keep the foothold overhead empty and the upper edge open.';if((m.climbs||[]).length>=4096)return 'Maximum roof climbs reached.';m.climbs=[...(m.climbs||[]).filter(q=>!(q.x===x&&q.y===y&&q.z===z&&q.dx===p.dx&&q.dy===p.dy)),p];return '';}
