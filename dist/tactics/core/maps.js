...
// A ramp crest crosses boundaries authored on either logical level.
function rampCrestBlocked(map,a,b){return [Math.min(levelOf(a),levelOf(b)),Math.max(levelOf(a),levelOf(b))].some(z=>blockedEdge(map,{...a,z},{...b,z}));}
export function neighbors(m,p,stairs=stairSet(m),includeDiagonals=true){
 ...
 for(const dz of [-1,1])if(stairs.has(stairKey(p.x,p.y,Math.min(z,z+dz)))&&passable(m,{x:p.x,y:p.y,z:z+dz}))out.push({x:p.x,y:p.y,z:z+dz,cost:stairs.get(stairKey(p.x,p.y,Math.min(z,z+dz)))});for(const link of roofNeighbors(m,p))out.push(link);return out.filter(b=>rampMoveAllowed(m,p,b)).concat(rampLinks(m,p).filter(b=>passable(m,b)&&!rampCrestBlocked(m,p,b)));
}
...
export function validateMap(raw,{connectivity=true}={}){
 ...
 if(!Array.isArray(raw.guards)||raw.guards.length>MAX_GUARDS||raw.guards.some(g=>!point(g)||!SPECIES.includes(g.species)||!WEAPON_IDS.includes(g.weapon)||(g.outfit!==undefined&&!['normal','red-hats'].includes(g.outfit)&&!(g.species==='donkey'&&g.outfit==='blue-hawaiian'))))return [...errors,'Use at most 46 guards (50 characters including the squad).'];
 ...
 if(raw.climbs!==undefined&&(!Array.isArray(raw.climbs)||raw.climbs.length>4096||raw.climbs.some(p=>!point(p)||(p.kind!==undefined&&p.kind!=='cliff')||!Number.isInteger(p.z)||p.z>=2||!Number.isInteger(p.dx)||!Number.isInteger(p.dy)||Math.abs(p.dx)+Math.abs(p.dy)!==1)))return [...errors,'Invalid roof climb: adjacent tiles and exactly one level required.'];
 ...
 errors.push(...rampErrors(raw),...cliffMapErrors(raw));for(const p of raw.props||[])if(isRamp(p)){const r=rampInfo(p);if(!passable(raw,r.entry)||!passable(raw,r.exit)||rampCrestBlocked(raw,r.high,r.exit))errors.push("Ramp approaches and upper connection must be unobstructed.");}const propPositions=new Set();for(const p of raw.props||[])for(const q of propCells(p)){const k=tileKey(q.x,q.y,q.z);if(propPositions.has(k))errors.push('Overlapping prop footprints.');propPositions.add(k);if(!inBounds(q.x,q.y,q.z)||!(floorTerrain(terrainAt(raw,q.x,q.y,q.z))||isCliff(p)&&terrainAt(raw,q.x,q.y,q.z)==='water'))errors.push('Props need supported floor tiles across their whole footprint.');}
 ...
 errors.push(...characterErrors(raw));for(const p of raw.starts)if(p.species!==undefined&&!SPECIES.includes(p.species)||p.weapon!==undefined&&!WEAPON_IDS.includes(p.weapon)||p.outfit!==undefined&&!['normal','red-hats'].includes(p.outfit)&&!(p.species==='donkey'&&p.outfit==='blue-hawaiian'))errors.push('Unsupported squad appearance or weapon.');const positions=new Set();for(const p of [...raw.starts,...raw.guards]){const k=tileKey(p.x,p.y,levelOf(p));if(positions.has(k))errors.push(`Overlapping unit starts at ${k}.`);positions.add(k);if(p.towerPost?!towerForUnit(raw,p):!passable(raw,p))errors.push(`Unit start needs a walkable floor or valid tower post at ${k}.`);}
