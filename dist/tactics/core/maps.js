...
export function validateMap(raw,{connectivity=true}={}){
 ...
 errors.push(...cliffMapErrors(raw));const propPositions=new Set();for(const p of raw.props||[])for(const q of propCells(p)){const k=tileKey(q.x,q.y,q.z);if(propPositions.has(k))errors.push('Overlapping prop footprints.');propPositions.add(k);if(!inBounds(q.x,q.y,q.z)||!(floorTerrain(terrainAt(raw,q.x,q.y,q.z))||isCliff(p)&&terrainAt(raw,q.x,q.y,q.z)==='water'))errors.push('Props need supported floor tiles across their whole footprint.');}
