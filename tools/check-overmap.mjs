...
try{
 ...
 const oldDrawing=await page.locator('#overmap').innerHTML(),oldSeed=await page.locator('#world-seed').inputValue();await page.locator('#randomize-tutorial').click();await page.waitForFunction(()=>document.getElementById('status').textContent.startsWith('World generated'));assert.notEqual(await page.locator('#world-seed').inputValue(),oldSeed);assert.notEqual(await page.locator('#overmap').innerHTML(),oldDrawing);assert.match(await page.locator('#generation-report').innerText(),/Strategic checks pass/);assert.equal(await page.locator('#tutorial-step').inputValue(),'1');await page.locator('#undo').click();assert.match(await page.locator('#generation-report').innerText(),/Drawing study/);await page.locator('#demo').click();
