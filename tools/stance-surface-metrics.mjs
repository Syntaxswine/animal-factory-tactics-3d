...
export function stanceSurfaces(worker){
 const result={body:Infinity,belly:Infinity,weapon:Infinity,knees:[Infinity,Infinity],feet:[Infinity,Infinity]},v=new T.Vector3();
 const hipIndex=worker.bones.findIndex(b=>b.name==='hips'),hip=hipIndex<0?new T.Vector3(NaN,NaN,NaN):new T.Vector3().setFromMatrixPosition(worker.skeleton.boneInverses[hipIndex].clone().invert());
 ...
 for(const part of worker.parts){const a=part.geometry.attributes.position;
  for(let i=0;i<a.count;i++){part.getVertexPosition(i,v);v.applyMatrix4(part.matrixWorld);result.body=Math.min(result.body,v.y);
   ...
   if(/overalls|trouser|waistcoat|shirt/.test(part.name)&&a.getY(i)>hip.y-.05&&a.getY(i)<hip.y+.35&&a.getX(i)>hip.x+.12)result.belly=Math.min(result.belly,v.y);
  }
 }
