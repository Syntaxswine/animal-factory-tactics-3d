 ...
 for(const outfit of ['normal','red-hats']){
  ...
  for(const reduced of [false,true]){
   ...
   for(const unavailable of [false,true,false]){
    // The refined horse reaches the old 1.4-high endpoint; use a closer half-tile endpoint on the visible floor.
    const playback=page.evaluate(({unavailable,reduced})=>new Promise(resolve=>{
     const r=battle3d.renderer,s=battle3d.state,u=s.units[1],x=u.x+(unavailable?.5:6),y=u.y,h=(unavailable?1.4:.48)*1.8/1.65;
