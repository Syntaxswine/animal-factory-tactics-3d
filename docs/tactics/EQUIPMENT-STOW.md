...
Stowing switches at action boundaries. Drawing now has a shared 850 ms transition documented in `EQUIPMENT-DRAW.md`; changing weapons skips the old weapon’s stow animation. It is separate from unequipping an item into the inventory. One body pose controller and one equipment-stow controller should own an actor at a time. Replacing the equipped model during a climb cancels that presentation safely.
