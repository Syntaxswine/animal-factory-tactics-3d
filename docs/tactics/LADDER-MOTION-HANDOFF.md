...
- `motion.duration` is authored presentation timing, **not an action-point cost**. `motion.phases` provides phase labels and start/end times. Diagnostics return local and transformed contact positions, planted flags and the posed root. These are art diagnostics, not gameplay collision authority.
