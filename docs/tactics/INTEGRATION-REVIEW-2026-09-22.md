...
## Character dossier, gameplay ladder playback and casualty integration

Approved editor work through `56beefc` and casualty work `f3189fd` together. The character dossier passes portrait/inventory, roster navigation, mobile layout, focus restoration and pause restoration browser checks. Unassigned attributes remain explicitly blank. Gameplay ladder ascent/descent pass with no duplicate AP/time charges and restore ordinary posing afterward. Playback is restricted to rifle-equipped non-pig mammals on the iron searchlight ladder tower; unsupported cases retain immediate traversal.
